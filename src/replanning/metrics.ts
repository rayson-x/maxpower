...
export function deriveMetricRegistry(input: {
  ...
  return [
    ...
    common("phase_progress", phaseDays, phase ? "moderate" : "low", phase ? { direction: phaseDays >= phaseLength ? "improving" : "stable", score: phaseDays / phaseLength } : { direction: "unknown" }, phase ? [`goal_cycle:${phase.id}`] : [], phase ? [] : ["no_active_phase"]),
