...
export function deriveMetricRegistry(input: {
  ...
  const performed = input.domain.workouts.flatMap((workout) => workout.setOutcomes.filter((outcome) => outcome.source === "user_confirmed" || outcome.source === "camera_confirmed").map((outcome) => ({ workout, outcome })));
  const phase = [...input.domain.goalCycles].sort((left, right) => right.revision - left.revision)[0]?.value.phasePath?.find((candidate) => candidate.startDate <= input.endDate && candidate.endDate >= input.startDate);
  ...
  const phaseDays = phase ? Math.max(0, Math.floor((Date.parse(`${input.endDate}T12:00:00Z`) - Date.parse(`${phase.startDate}T12:00:00Z`)) / 86_400_000) + 1) : 0;
  const phaseLength = phase ? Math.max(1, Math.floor((Date.parse(`${phase.endDate}T12:00:00Z`) - Date.parse(`${phase.startDate}T12:00:00Z`)) / 86_400_000) + 1) : 0;
  return [
    ...
    common("phase_progress", phase ? Math.min(1, phaseDays / phaseLength) : 0, phase ? "moderate" : "low", phase ? { direction: phaseDays >= phaseLength ? "improving" : "stable", score: phaseDays / phaseLength } : { direction: "unknown" }, phase ? [`goal_cycle:${phase.id}`] : [], phase ? [] : ["no_active_phase"]),
