/** Incremental SHA-256 for large native files that must not be copied into one JS buffer. */
export class IncrementalSha256 {
  ...
  update(bytes: Uint8Array): this {
    ...
    if (!Number.isSafeInteger(nextLength) || !Number.isSafeInteger(nextLength * 8)) {
      throw new Error("sha256_input_too_large");
    }
