/** Incremental SHA-256 used by local account namespace derivation. */
export class IncrementalSha256 {
  ...
  update(bytes: Uint8Array): this {
    ...
    const nextLength = this.bytesHashed + bytes.byteLength;
    if (!Number.isSafeInteger(nextLength) || !Number.isSafeInteger(nextLength * 8)) throw new Error("sha256_input_too_large");
    this.bytesHashed = nextLength;
    let offset = 0;
  ...
  private transform(block: Uint8Array): void {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      this.words[index] = (((rotateRight(x, 7) ^ rotateRight(x, 18) ^ (x >>> 3)) + this.words[index - 16]) +
        ((rotateRight(y, 17) ^ rotateRight(y, 19) ^ (y >>> 10)) + this.words[index - 7])) >>> 0;
    }
    let a = this.state[0]; let b = this.state[1]; let c = this.state[2]; let d = this.state[3];
    let e = this.state[4]; let f = this.state[5]; let g = this.state[6]; let h = this.state[7];
    for (let index = 0; index < 64; index += 1) {
      const sigma1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25);
      const choose = (e & f) ^ (~e & g);
      const temp1 = (h + sigma1 + choose + CONSTANTS[index] + this.words[index]) >>> 0;
