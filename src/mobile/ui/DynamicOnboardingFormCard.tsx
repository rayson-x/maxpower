...
function groupFieldLabel(name: string): string {
  return ({ exercise_variant: "动作变式", load: "重量", reps: "次数", effort_metric: "强度记录方式", effort_value: "强度数值", performed_on: "训练日期", conditions: "当时情况", days_per_week: "每周可训练天数", minutes_per_session: "每次可训练分钟", consecutive_weeks: "最近连续训练周数", usual_sessions_per_week: "这段时间每周通常训练次数", time_away_weeks: "此前中断周数" } as Record<string, string>)[name] ?? name;
}
...
const styles = StyleSheet.create({
  ...
  input: { minHeight: 46, borderRadius: 12, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.paper, color: colors.ink, paddingHorizontal: 11, fontSize: 14, fontWeight: "600" }, multiline: { minHeight: 88, paddingTop: 11, textAlignVertical: "top" }, metricHelp: { marginTop: 6, color: colors.ink3, fontSize: 11, lineHeight: 17 },
