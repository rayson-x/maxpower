...
- [ ] 相机界面只突出动作、组号、用户提供的重量、目标次数、confirmed reps、构图状态和最多一条当前观察。
...
- [ ] 手动与 Realtime 路径进入同一 Set Review；用户确认 actual load、performed reps、RIR 和可选主观状态后才写入结果。
- [x] 用户修正次数只改变 performed value，原始 observed dispositions、evidence lineage 和 correction provenance 保持可追溯。
- [ ] camera、LLM 和 motion adapter 均不能产生 actual load、RIR、疼痛、肌肉激活或伤害判断；未知保持 unknown/cannot judge。
- [ ] 骨架、角度、FPS、runtime 与模型信息默认不占据训练界面，只在组后详情或独立诊断入口可见。
