...
/** S1 知识包构建缝：语料 passages 进包、tier/experimental 标记、sourceRef 钉版、检索命中。 */

test("教练语料 passages 进包：tier D + experimental + 钉语料文档", () => {
  ...
  for (const passage of corpus) {
    ...
    assert.equal(passage.experimental, true, "dogfood 期必须带 experimental 标记");
  }
  // 关键模式条目都在
  ...
  assert.ok(gists.length > 0 && gists.every((gist) => gist.experimental === true), "蒸馏 gist 必须继承 experimental");
});
...
test("四份核验调研进包：结论落地建议可查，内部裁决与引用清单剔除", () => {
  ...
  for (const [path, title] of [["fat-loss-plateau", "减脂平台期证据"], ["body-recomposition", "身体重组证据"], ["detraining-retraining", "停训与复练证据"], ["muscle-recovery-windows", "肌群恢复时间窗证据"]] as const) {
    ...
    assert.ok(hits.every((passage) => passage.experimental === true), `${title} 新内容必须标 experimental（dogfood 期）`);
  }
});
