...
- **Recovery constraint**: a graded, time-bounded limit derived from recovery facts. It constrains planning as an advisory constraint presented to the Agent and the user — it shapes default composition quality, never the user's right to choose; the user may confirm through it after hearing the trade-off. It does not itself choose exercises, sets, repetitions, or load.
