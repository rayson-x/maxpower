...
Server boundary for MaxPower identity, authenticated text LLM inference, metering, and account lifecycle. Product records remain on the client and this package does not import the Expo application.
...
- PostgreSQL for identity, LLM entitlements, content-free usage accounting, and deletion jobs.
- An OpenAI-compatible provider behind the stable text-only alias `maxpower/coach-v1`; physical endpoints, models, and credentials stay server-side.
...
- All public auth, provider, and CORS URLs must be HTTPS. PostgreSQL must request TLS and both Redis URLs must use `rediss://`.
- `AUTH_SECRET`, provider keys, OAuth secrets, database credentials, Redis credentials, and `LLM_FINGERPRINT_SECRET` belong in the deployment secret manager. Never place them in an image, repository, log, APM attribute, or error report.
...
- The text alias has required `MAX_INPUT_BYTES`, `MAX_INPUT_TOKENS`, and `MAX_OUTPUT_TOKENS` limits. Startup computes the worst-case credit reservation from those caps; requests that exceed them stop before the provider, and the provider request is clamped to the configured output-token ceiling.
...
`GET /healthz` checks the process. `GET /readyz` requires PostgreSQL and both Redis roles. A failed dependency returns only `503 {"status":"not_ready"}` without the dependency error.
...
`POST /v1/me/deletion` immediately marks the account pending deletion and revokes service access. The idempotent worker removes LLM accounting metadata, sessions, and Better Auth identity rows.
