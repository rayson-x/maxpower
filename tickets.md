...
- [ ] 杠铃划船使用版本化运动学 profile 声明分期信号、极点、相位、指标关节和支持机位
- [ ] 一个高层分析入口组合 rep 分割、指标提取和规则评分
- [ ] 输出包含 rep、逐字段质量、规则四态、分数/partial、覆盖、profile 和 rule 版本
- [ ] 组后 UI 展示动作、机位、逐 rep 指标、实验状态和“未检出明显问题”文案
- [ ] 每个 rep 可以定位到对应视频时间段
- [ ] 同一真实 fixture 在 UI 数据层与 harness 得到相同结构化结果
- [ ] 用户选择杠铃划船时覆盖低置信自动识别建议
...
- [ ] 引体向上、高位下拉、坐姿划船和直臂下压各有版本化 profile
- [ ] 每个动作至少一个现有 fixture 或明确记录的测试样本贯通高层分析入口
